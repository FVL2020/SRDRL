...
def main():
    opt = option.parse("options/train/train_srdrl.json", is_train=True)
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0:
                ...
                for val_data in val_loader:
                    ...
                    save_SR_path = opt['path']['root'] + 'validations/' + opt['name'] + '/{:d}/'.format(current_step)
                    util.mkdir(save_SR_path)
                    save_SR_img_path = os.path.join(save_SR_path, '{:s}.png'.format( \
                        img_name))
