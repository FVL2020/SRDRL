...
def main():
    ...
    folder_GT = './GroundTruth/BSDS100_HR'
    folder_Gen = './Results/BSDS100_SR'
    print(folder_Gen)
    ...
    Gen_list = sorted(glob.glob(folder_Gen + '/*'))
    downsample_num = len(Gen_list)/len(GT_list)
    #print('Downsample num = ', downsample_num)
    ...
    for i, img_path in enumerate(Gen_list):
        base_name = os.path.splitext(os.path.basename(img_path))[0]
        im_GT = cv2.imread(GT_list[i % len(GT_list)])/ 255.
        im_Gen = cv2.imread(img_path)/ 255.
        if im_GT.shape != im_Gen.shape:
            #continue
            height, width = im_Gen.shape[:2]
            im_GT = im_GT[0:height, 0: width]
